...
import { Camera, Video, Palette, PenTool } from 'lucide-react';
...
const skills = [
  ...
  { name: 'Brand Design', icon: PenTool },
];
...
const About = () => {
  return (
    <section id="about" className="relative w-full min-h-screen py-24 flex items-center bg-brand-surface z-20">
      ...
      {/* Film perforation strip — decorative left border */}
      <div className="absolute left-0 top-0 bottom-0 w-8 hidden lg:flex flex-col items-center justify-center gap-6 opacity-[0.07]">
        {Array(20).fill(null).map((_, i) => (
          <div key={i} className="w-3 h-5 rounded-sm border border-brand-gold bg-brand-gold/10" />
        ))}
      </div>
      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          ...
          <div className="w-full lg:w-3/5 order-2 lg:order-1">
            ...
            >
              Hii, I'm Akshat Dange. An ENTC Third year engineering student and a freelance photographer and content creator.
            ...
            {/* Skills with camera-inspired icons */}
              ...
              {skills.map((skill, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center gap-3 px-5 py-3 border border-white/10 rounded-full text-sm tracking-wide text-brand-text/80 glass-panel group hover:border-brand-gold/30 transition-colors duration-300"
                  >
                    <Icon size={14} className="text-brand-gold/70 group-hover:text-brand-gold transition-colors" />
            ...
            {/* Gear / tech info — subtle photographer credibility */}
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              {['Nikon', 'Adobe Lightroom', 'Premiere Pro', 'DaVinci Resolve'].map((tool) => (
                <span key={tool} className="font-mono text-[10px] tracking-[0.15em] uppercase text-brand-text/25">
                  {tool}
                </span>
              ))}
            </motion.div>
              ...
              {PROFILE_IMG ? (
                <div className="polaroid-frame">
                  ...
                  <div className="mt-2 text-center">
                    <span className="font-mono text-[9px] tracking-[0.2em] uppercase text-brand-text/20">
                      Self Portrait · 2024
                    </span>
                  </div>
                </div>
